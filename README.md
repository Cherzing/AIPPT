...
<p align="center">
  AIPPT 是一款可私有化部署的 AI 演示文稿生成与编辑平台。
</p>
...
AIPPT 面向真实办公场景，提供从主题、文档、大纲到可编辑 PPT 的完整生成链路。它不是只输出静态预览图，而是围绕“生成、编辑、复用、导出、部署”构建的演示文稿工作台。

用户可以先选择模板，再生成演示文稿；也可以在浏览器中继续编辑文字、图片、形状、表格、图表等元素，最后导出为 PPTX 或 PDF。项目适合企业汇报、项目材料、行业模板、培训课件、产品介绍、技术方案和私有化办公自动化场景。

本项目基于 [presenton/presenton](https://github.com/presenton/presenton/) 继续扩展，重点增强了中文办公体验、模板分类、用户权限、模型配置、Docker 部署、AI 图片编辑和部分模板的原生可编辑模型能力。
...
- 支持在生成前编辑大纲，先确认内容结构再进入页面生成。
- 支持模板驱动生成，保留模板风格与版式约束。
- 支持生成过程状态展示和历史文稿管理。
- 支持对已生成文稿继续编辑和二次迭代。
...
这样生成后的 PPT 更接近真实办公文件：用户可以在网页中修改，也可以导出后继续在 PowerPoint、WPS 或 Keynote 中加工。
