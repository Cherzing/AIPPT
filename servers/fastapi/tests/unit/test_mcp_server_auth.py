...
def test_is_mcp_server_enabled_in_server_deployments(monkeypatch):
    monkeypatch.setattr(mcp_server, "is_aippt_electron_desktop", lambda: False)

    assert mcp_server.is_mcp_server_enabled() is True


def test_is_mcp_server_disabled_in_electron_desktop(monkeypatch):
    monkeypatch.setattr(mcp_server, "is_aippt_electron_desktop", lambda: True)
...
def test_create_mcp_auth_provider_enabled_when_auth_configured(monkeypatch):
    ...
    assert isinstance(provider, mcp_server.AIPPTTokenVerifier)


def test_AIPPT_token_verifier_accepts_valid_token(monkeypatch):
    monkeypatch.setattr(
        mcp_server,
        "validate_session_token",
        lambda token: "admin" if token == "valid-token" else None,
    )
    verifier = mcp_server.AIPPTTokenVerifier()
...
def test_AIPPT_token_verifier_rejects_invalid_token(monkeypatch):
