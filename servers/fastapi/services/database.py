...
def _ensure_compatible_schema(sync_conn):
    ...
    if "presentations" not in table_names:
        return

    presentation_columns = {
        column["name"] for column in inspector.get_columns("presentations")
    }
    if "owner_user" not in presentation_columns:
        sync_conn.execute(text("ALTER TABLE presentations ADD COLUMN owner_user VARCHAR"))
