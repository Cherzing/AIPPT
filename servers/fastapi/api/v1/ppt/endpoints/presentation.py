...
from utils.presentation_access import (
    get_request_role,
...
async def get_all_presentations(
    ...
    if get_request_role(request) != "admin":
        query = query.where(PresentationModel.owner_user == get_request_username(request))
...
async def get_presentation(
    ...
    if not presentation:
        raise HTTPException(404, "未找到演示文稿")
    require_presentation_access(presentation, request)
...
async def create_presentation(
    ...
    presentation = PresentationModel(
        id=presentation_id,
        content=content,
        owner_user=request_owner_username(request),
...
async def prepare_presentation(
    ...
    if not presentation:
        raise HTTPException(status_code=404, detail="未找到演示文稿")
...
async def generate_presentation_handler(
    ...
    try:
        ...
        if not isinstance(e, HTTPException):
            traceback.print_exc()
            e = HTTPException(status_code=500, detail="演示文稿生成失败")
...
async def generate_presentation_sync(
    ...
    try:
        ...
        return await generate_presentation_handler(
            ...
            presentation_id,
            None,
            owner_user=request_owner_username(request_http),
        ...
        traceback.print_exc()
        raise HTTPException(status_code=500, detail="演示文稿生成失败")
...
async def check_async_presentation_generation_status(
    ...
    if not status:
        raise HTTPException(
            status_code=404, detail="未找到演示文稿生成任务"
        )
    return status
...
async def edit_presentation_with_new_content(
    ...
    require_presentation_access(presentation, request_http)
...
async def derive_presentation_from_existing_one(
    ...
    new_presentation.owner_user = request_owner_username(request_http)
    new_slides = []
    for each_slide in slides:
        updated_content = None
        new_slide_data = list(
