...
import { setAuthUser, setCanChangeKeys, setLLMConfig } from '@/store/slices/userConfig';
...
export function ConfigurationInitializer({ children }: { children: React.ReactNode }) {
  ...
  const fetchUserConfigState = async () => {
    ...
    let isAdmin = false;
    try {
      const authResponse = await fetch(getApiUrl("/api/v1/auth/status"), {
        method: "GET",
        cache: "no-store",
        credentials: "include",
      });
      if (authResponse.ok) {
        ...
        const role = authStatus?.role === "admin" ? "admin" : "user";
        isAdmin = role === "admin";
        dispatch(setAuthUser({
          username: typeof authStatus?.username === "string" ? authStatus.username : null,
          role,
        }));
      }
    } catch (e) {
      console.error("Failed to fetch auth status:", e);
      dispatch(setAuthUser({ username: null, role: null }));
    }
    ...
    canChangeKeys = canChangeKeys && isAdmin;
