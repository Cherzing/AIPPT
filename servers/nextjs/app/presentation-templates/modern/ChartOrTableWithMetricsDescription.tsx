...
const tractionSchema = z.object({
  ...
  tableColumns: z.array(z.string().min(1).max(40)).min(2).max(10).default(["指标", "Value"]),
...
function computeStats(
  ...
  return seriesKeys.map((key) => {
    ...
    const label = formatSeriesLabel(key);
    return {
      label,
      value: `${growth >= 0 ? "+" : ""}${Math.round(growth)}% 增长`,
      description: `${label}在统计周期内的增长情况。`,
    };
  });
}
