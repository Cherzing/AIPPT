...
const ThemePanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadCustomThemes = async () => {
      try {
        ...
        notify.error(
          '无法加载主题',
          error?.message || '无法加载已保存主题，内置主题仍可使用。'
        )
      }
    }
  ...
  const saveAsCustom = async () => {
    ...
    try {
      ...
      notify.success('主题已保存', '新主题已创建，可立即使用。')
    } catch (error: any) {
  ...
  const handleDelete = async (themeId: string) => {
    try {
      ...
      notify.success("主题已删除", "该主题已从你的主题库中移除。")
    } catch (error: any) {
  ...
  const renderFontStep = () => (
    ...
    >
      ...
      {userFonts.fonts.length > 0 && (
        <div className="px-2.5">
          <p className='text-xs text-[#4C4C4C] font-medium text-end pb-1.5'>你上传的字体</p>
                ...
                <div className='pt-7  w-full  h-[calc(90vh-120px)] flex flex-col justify-between  '>
                  ...
                  <div className='border-t border-[#EDEEEF] py-6 '>
                    <div className='flex justify-end px-5 gap-2'>
                      ...
                      >
                        {currentStep === 1 ? '生成主题调色板' : currentStep === 2 ? '继续设置字体' : currentStep === 3 ? '继续设置标识' : '保存为自定义主题'}
                        <ChevronRight className='w-4 h-4' />
                      </button>
                    </div>
                  </div>
