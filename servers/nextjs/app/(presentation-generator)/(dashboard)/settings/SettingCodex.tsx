...
export default function CodexConfig({
    ...
    const handleSignIn = async () => {
        try {
            ...
            if (!res.ok) throw new Error("无法启动认证");
    ...
    const handleManualExchange = async () => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(err.detail || "验证码交换失败");
            }
            const data = await res.json();
            stopPolling();
            setAuthStatus("authenticated");
        ...
        return (
            <div className="space-y-4">
                <div className="flex items-center gap-3 py-2">
                    ...
                    <span className="text-sm text-gray-600">等待登录完成…</span>
                    <button
    ...
    if (authStatus === "authenticated") {
        const planLabel = isPro === true ? "Pro" : isPro === false ? "免费版" : "未知";
