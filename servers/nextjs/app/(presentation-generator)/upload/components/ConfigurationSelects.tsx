  ...
  return (
    <Popover open={open} onOpenChange={onOpenChange}>
      ...
      <PopoverContent className="w-[140px] p-0 font-syne" align="end">
        <div className="sticky top-0 z-10 bg-white p-2 border-b" onMouseDown={(e) => e.stopPropagation()} onPointerDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center gap-2">
            <Input
              ...
              onChange={(e) => setCustomInput(sanitizeToPositiveInteger(e.target.value))}
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  applyCustomValue();
  ...
  <Popover open={open} onOpenChange={onOpenChange}>
    <PopoverTrigger asChild>
      <button
        ...
        aria-expanded={open}
        className="w-[140px] flex items-center bg-white gap-2 overflow-hidden h-[34px] font-syne font-semibold text-[#191919] rounded-full px-3.5 ring-1 ring-inset ring-slate-200 shadow-sm"
      >
        <Languages className="w-3.5 h-3.5" />
        <span className="w-[72px] text-left">
          <span className="text-xs font-medium truncate block">{value || "选择语言"}</span>
        </span>
        <ChevronUp className="ml-2 h-4 w-4 shrink-0" />
      </button>
    </PopoverTrigger>
    <PopoverContent className="w-[300px] p-0" align="end">
      <Command>
        <CommandInput placeholder="搜索语言..." className="font-instrument_sans" />
        <CommandList>
          ...
          <CommandGroup>
            {Object.values(LanguageType).map((language) => (
              ...
              >
                <Check className={cn("mr-2 h-4 w-4", value === language ? "opacity-100" : "opacity-0")} />
                {language}
              </CommandItem>
            ))}
          </CommandGroup>
