...
const SupportingDoc = ({
    ...
    const handleValidate = (filesToReview: File[]) => {
        ...
        if (disallowed.length > 0) {
            notify.error('部分文件不受支持', '支持：Word、PowerPoint、表格、PDF/TXT 和图片文件。')
        }
    }
    ...
    const applyFileLimit = (candidateFiles: File[]) => {
        ...
        notify.warning('已达到文件数量上限', `You can upload up to ${MAX_SUPPORTED_FILES} documents only.`)
        ...
        <div className="space-y-2" data-testid="attachments-uploader">
            ...
            >
                ...
                <div className="flex flex-col items-center gap-2">
                    ...
                    <p className='text-[#808080] text-sm  font-normal'>（Office 文档、表格、图片、PDF/TXT）</p>
                </div>
            </label>
                ...
                <div className="mt-2">
                    ...
                    {filteredFiles.length !== files.length && (
                        <p className="mt-2 text-xs text-amber-600 font-syne">
                            Some files were skipped. 支持：Word、PowerPoint、表格、PDF/TXT 和图片文件。
                        </p>
                    )}
                </div>
            )}
