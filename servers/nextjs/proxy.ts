...
async function getAuthStatus(request: NextRequest): Promise<AuthStatus> {
  ...
  try {
    ...
    if (!response.ok) {
      return { configured: true, authenticated: false, role: null };
    }
...
export async function proxy(request: NextRequest) {
  ...
  if (pathname === "/settings" || pathname.startsWith("/settings/")) {
    ...
    if (!authStatus.authenticated) {
      const redirectUrl = request.nextUrl.clone();
      redirectUrl.pathname = "/";
      redirectUrl.search = "?reason=unauthorized";
      return NextResponse.redirect(redirectUrl);
    }
    if (authStatus.role !== "admin") {
      const redirectUrl = request.nextUrl.clone();
      redirectUrl.pathname = "/dashboard";
      redirectUrl.search = "?reason=admin-required";
      return NextResponse.redirect(redirectUrl);
    }
    return NextResponse.next();
  }
...
export const config = {
  matcher: ["/api/:path*", "/pdf-maker", "/settings/:path*"],
};
