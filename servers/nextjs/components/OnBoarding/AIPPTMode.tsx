...
const OTHER_PROVIDERS = Object.values(LLM_PROVIDERS).filter(
    (provider) => provider.value !== "codex" && provider.value !== "chatgpt" && !LOCAL_PROVIDERS.includes(provider.value)
);
const OTHER_PROVIDER_VALUES = new Set(OTHER_PROVIDERS.map((provider) => provider.value));
type TextProviderTab = "local" | "other";
...
const AIPPTMode = ({
    ...
    const [textProviderTab, setTextProviderTab] = useState<TextProviderTab>(getTextProviderTab(userConfigState.llm_config.LLM));
        ...
        <div className='w-full max-w-[660px] font-syne pb-10'>
            ...
            <div className='p-3 border border-[#EDEEEF] rounded-[11px] bg-white '>
                ...
                <Tabs
                    ...
                    className="w-full"
                >
                    <TabsList className="grid h-14 w-full grid-cols-2 rounded-[10px] border border-[#EDEEEF] bg-[#F6F6F9] p-1 shadow-inner shadow-black/[0.02]">
                ...
                <div className="mt-6 flex w-full max-w-[300px] flex-col items-start gap-4">
                    <div className="relative flex w-full flex-col justify-end items-start">
                        ...
                        {!isManualModelProvider && llmConfig.LLM !== 'ollama' && (!modelsChecked || availableModels.length === 0) && (
