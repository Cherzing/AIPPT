  ...
  const fetchOllamaModels = useCallback(async () => {
    ...
    try {
      ...
      notify.success(
        "已连接 Ollama",
        pulled.length > 0
          ? `已找到 ${pulled.length} 个已下载模型。库中还有 ${libraryOnly.length} 个可用模型。`
          : "Ollama 可访问。可浏览模型库并下载模型。"
      );
      ...
      if (!reachable.usedFallback && requestedUrl && reachable.resolvedUrl !== requestedUrl) {
        notify.success(
          "已更新 Ollama URL",
          `使用 ${reachable.resolvedUrl} 进行 Ollama 检查。`
        );
      }
    } catch (error) {
      setCombinedModels([]);
      ...
      onInputChange("", "ollama_model");
      setModelsCheckError(
        error instanceof Error ? error.message : "请检查 Ollama URL 后重试。"
      );
      notify.error(
        "无法连接 Ollama",
        error instanceof Error ? error.message : "请检查 Ollama URL 后重试。"
        ...
        (event: OllamaPullProgressEvent) => {
          ...
          switch (event.type) {
            case "status":
              ...
              break;
            case "progress":
              setPullStatus(event.status || "正在下载...");
            ...
            case "complete":
              ...
              notify.success("模型已下载", `${modelName} 可以使用。`);
              break;
            case "error":
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-gray-500">
          生成所必需。请使用 {defaultOllamaUrl}
          {!isElectronRuntime
            ? "，或者点击“检查模型”，在 Ollama 与当前容器同机运行时自动检测 localhost。"
            : "."}
        </p>
        <Button
          type="button"
      ...
      {modelsChecked && combinedModels.length === 0 && (
        <p className="text-sm text-gray-500">
          {modelsCheckError
            ? modelsCheckError
            : "Ollama 可访问，但尚未安装模型。请在 Ollama 中拉取模型后重新检查。"}
        </p>
      )}
