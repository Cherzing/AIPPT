/**
 * Docker / startup terminal banner for AIPPT.
...
function loadAsciiBanner() {
  ...
  const asciiPath = path.join(thisDir, "aippt-ascii.txt");
 ...
 */
export function printAIPPTStartupBanner(opts = {}) {
  ...
  const title = [
    "",
    BOLD + fgRgb(138, 99, 255, "   AIPPT 智能演示文稿生成平台"),
    ...(mode === "development"
      ? [
          "   " +
            ...
            BOLD +
            fgRgb(224, 218, 255, "https://github.com/AIPPT/AIPPT"),
        ]
      : []),
    muted("   ─────────────────────────────────────────────────────────"),
    "",
  ...
  const summaryLines =
    mode === "development"
      ...
      : [
          pipe(padVis("  " + BOLD + "应用访问地址" + RESET, W)),
          boxDivider,
          pipe(padVis("  " + muted("Mode:             ") + mode, W)),
          pipe(
            padVis(
