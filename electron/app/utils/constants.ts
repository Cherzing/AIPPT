...
const appDirectoryName = "AIPPT Open Source";
...
function getAppDataBaseDir(tempRoot: string): string {
  const home = getHomeDir();
  const fallback = path.join(tempRoot, "AIPPT-app-data");
...
function setElectronPath(name: string, dir: string): void {
  try {
    ...
    console.warn(`[AIPPT] Failed to set Electron path ${name}=${dir}`, error);
  }
}
...
function appendDiskCacheSwitch(cacheDir: string): void {
  try {
    ...
    console.warn("[AIPPT] Failed to configure Chromium disk cache path", error);
  }
}
...
export function initializeAppPaths(): ElectronAppPaths {
  ...
  const userDataDir = firstWritableDirectory(
    unique([
      ...
      path.join(tempRoot, "AIPPT-user-data"),
    ]),
    "user data"
  );
  ...
  const tempDir = firstWritableDirectory(
    unique([path.join(tempRoot, "AIPPT"), path.join(userDataDir, "temp")]),
    "temporary"
  );
