...
function getRecoveryMode(): ChromiumCacheRecoveryStatus["mode"] {
  const raw = process.env.AIPPT_CHROMIUM_CACHE_RECOVERY?.trim().toLowerCase();
...
export function prepareChromiumCacheRecovery(
  ...
  if (status.errors.length > 0) {
    ...
    safeWarn("[AIPPT] Chromium cache recovery finished with errors:", status);
    return status;
  }

  safeLog("[AIPPT] Chromium cache recovery prepared:", {
    mode: status.mode,
    quarantined: status.quarantined,
    removedStaleDirectories: status.removedStaleDirectories,
  });
...
export async function finishChromiumCacheRecovery(
  ...
  try {
    ...
    safeError("[AIPPT] Chromium cache recovery failed:", error);
